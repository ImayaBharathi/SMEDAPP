...
describe('Starting tests for smedappfinal', function() {
  ...
  it('website title should be smedappfinal', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('smedappfinal');
    })
  });

  it('network-name should be networksmedapp@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('networksmedapp@0.0.1.bna');
    });
  });

  it('navbar-brand should be smedappfinal',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('smedappfinal');
    });
  });
